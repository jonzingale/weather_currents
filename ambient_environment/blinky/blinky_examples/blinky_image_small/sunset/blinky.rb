...
class Blinky
	...
	def blink(color_state,neigh)
		state = little_blue?(color_state) ? 0 : 1
		sum_neigh_state = neigh.map{|n| little_blue?(n) ? 0 : 1 }.inject :+

		# turn off
		state==0&&little_blue?(color_state)&&sum_neigh_state < 2 ? [0] * 3 << 100 :
		# turn on
		state==0&&sum_neigh_state==3 ? rgb_avg(neigh) :
		# don't change
		middle_blue?(color_state) ? color_state :
		# stay on
		state==1&&(sum_neigh_state==3||sum_neigh_state==2) ? color_state : color_state
	end

	def much_blue?(colors) ; colors[1] < 100 ; end
	def little_blue?(colors) ; colors.all?{|t|t < 100} ; end # 30
	def middle_blue?(colors) ; !(much_blue?(colors)||little_blue?(colors)) ; end
